...
def get_markdown(email):
    # retrive the markdown content from /data/format.md
    file = os.path.join(config["root"], "format.md")
...
def a2_format_markdown():
    ...
    original = get_markdown(config["email"])
    write_file("format.md", original)
